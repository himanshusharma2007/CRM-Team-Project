  ...
  try {
    ...
    if(email || phoneNo){
      ...
      if(await Contact.findOne({phoneNo})){
        return res.status(400).json({error: "Contact phone number already exists"});
      }
    }
    const contactData = await Contact.findByIdAndUpdate(req.params.id, {contactName, companyName, email, phoneNo}, {new: true});
